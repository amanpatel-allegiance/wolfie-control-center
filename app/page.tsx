...
export default async function OverviewPage() {
  ...
  return <section>
    <PageHeader title="Operations overview" description="Live health across every extraction and sync pipeline" actions={<><button className="ref-btn"><CalendarRange />Last 24 hours⌄</button><AutoRefreshButton/><Link href="/pipelines" className="ref-btn ref-btn-primary"><Play className="fill-current" />Run pipeline</Link></>}/>

    <div className="ref-health-banner surface"><span className="ref-health-good"><Check className="size-3.5" strokeWidth={3} /></span><strong>Platform {attention.length || alerts.length ? "requires attention" : "healthy"} · {withinSla} of {pipelines.length} pipelines within SLA</strong><div className="ref-health-links"><Link href="/pipelines?state=warning" className="ref-warn-link inline-flex items-center gap-1.5"><AlertTriangle className="size-3.5" />{attention.length} pipeline{attention.length === 1 ? "" : "s"} nearing or outside SLA</Link><Link href="/alerts" className="ref-danger-link inline-flex items-center gap-1.5"><CircleAlert className="size-3.5" />{alerts.length} incident{alerts.length === 1 ? "" : "s"} open</Link></div></div>

    <div className="ref-metrics"><MetricCard label="Healthy" value={kpis.healthy} hint={`${withinSla} within freshness SLA`} tone="healthy" icon={<CheckCircle2 className="size-4" />}/><MetricCard label="Running" value={kpis.running} hint={`${last24.length} runs in last 24h`} tone="running" icon={<Activity className="size-4" />}/><MetricCard label="SLA compliance" value={slaPct == null ? "—" : `${slaPct}%`} hint={`${withinSla} of ${pipelines.length} pipelines`} tone={slaPct != null && slaPct >= 90 ? "healthy" : "warning"} icon={<CircleGauge className="size-4" />}/><MetricCard label="Rows processed" value={formatNumber(rowsProcessed)} hint="Reported in last 24h" tone="running" icon={<Rows3 className="size-4" />}/><MetricCard label="Failed runs" value={failed24.length} hint={`${completed24.length} completed in last 24h`} tone={failed24.length ? "failed" : "healthy"} icon={<CircleX className="size-4" />}/></div>

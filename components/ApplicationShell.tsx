...
export function ApplicationShell({ children, email, role, alertCount }: { children: React.ReactNode; email: string; role: string; alertCount: number }) {
  ...
  return (
    <div className="control-center-shell min-h-screen">
      ...
      <div className="min-w-0 min-[821px]:pl-[232px]">
        <header className="sticky top-0 z-30 flex h-[58px] items-center gap-4 border-b border-white/10 bg-wolfie-navy px-4 min-[821px]:border-wolfie-border min-[821px]:bg-white min-[821px]:px-6">
          ...
          <form action="/pipelines" method="get" className="top-search hidden h-9 w-[min(440px,42vw)] items-center gap-2 rounded-lg border border-wolfie-border bg-white px-3 text-wolfie-muted min-[821px]:flex">
            ...
            <input ref={searchRef} name="q" className="min-w-0 flex-1 border-0 bg-transparent text-xs outline-none placeholder:text-wolfie-subtle" placeholder="Search pipelines, datasets, runs…" />
            <kbd className="rounded border border-wolfie-border px-1.5 py-0.5 text-[10px] text-wolfie-subtle">⌘K</kbd>
          </form>
          <div className="flex-1" />
          <details className="group relative"><summary className="topbar-pill inline-flex h-9 cursor-pointer list-none items-center gap-2 rounded-lg border border-[#33445A] bg-[#102033] px-3 text-[13px] font-semibold text-white min-[821px]:border-wolfie-border min-[821px]:bg-white min-[821px]:text-[#344054]"><i className="status-live-dot size-[7px] rounded-full bg-state-healthy" />Production<ChevronDown className="size-3.5 opacity-60 transition group-open:rotate-180" /></summary><div className="select-menu right-0 w-[260px]"><div className="px-2 py-2"><strong className="block text-xs text-wolfie-ink">Production workspace</strong><small className="mt-1 block text-[10px] leading-4 text-wolfie-muted">Connected to live Supabase telemetry with {role} access.</small></div><Link href="/pipelines" className="select-menu-item"><span><strong>Pipeline registry</strong><small>Browse connected production sources</small></span><Workflow className="size-4"/></Link><Link href="/incidents" className="select-menu-item"><span><strong>Operational incidents</strong><small>{alertCount} active conditions</small></span><AlertTriangle className="size-4"/></Link></div></details>
          <details className="group relative hidden sm:block"><summary className="topbar-pill inline-flex h-9 cursor-pointer list-none items-center gap-1.5 rounded-lg border border-wolfie-border bg-white px-3 text-[13px] font-semibold text-[#344054]"><Clock3 className="size-3.5 text-wolfie-muted" />GST (UTC+4)<ChevronDown className="size-3.5 opacity-60 transition group-open:rotate-180" /></summary><div className="select-menu right-0 w-[265px]"><div className="px-2 py-2"><strong className="block text-xs text-wolfie-ink">Operational timezone</strong><small className="mt-1 block text-[10px] leading-4 text-wolfie-muted">Schedules use Gulf Standard Time. Persisted event timestamps remain in UTC for audit consistency.</small></div><Link href="/schedules" className="select-menu-item"><span><strong>Open schedules</strong><small>Review cron and timezone ownership</small></span><CalendarClock className="size-4"/></Link><Link href="/runs" className="select-menu-item"><span><strong>Open run history</strong><small>Inspect relative and UTC event times</small></span><Clock3 className="size-4"/></Link></div></details>
          <Link href="/incidents" className="topbar-icon relative hidden size-9 place-items-center rounded-lg border border-wolfie-border bg-white text-wolfie-muted hover:text-wolfie-ink sm:grid" aria-label={`${alertCount} active incidents`}><Bell className="size-4" />{alertCount > 0 && <span className="absolute -right-1.5 -top-1.5 grid min-w-[17px] h-[17px] place-items-center rounded-full border-2 border-white bg-state-failed px-0.5 text-[9px] font-semibold text-white">{Math.min(alertCount, 99)}</span>}</Link>
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 min-[821px]:hidden" role="dialog" aria-modal="true" aria-label="Navigation">
